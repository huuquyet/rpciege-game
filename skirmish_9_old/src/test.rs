...
mod old_contract {
    soroban_sdk::contractimport!(
        file =
            "../target/wasm32-unknown-unknown/release/skirmish_9_old.wasm"
    );
}

mod new_contract {
    soroban_sdk::contractimport!(
        file = "../target/wasm32-unknown-unknown/release/skirmish_9_new.wasm"
    );
}
...
fn test() {
    ...
    assert_eq!(client.solve(&Some(admin)), ());
}